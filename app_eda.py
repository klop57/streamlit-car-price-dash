...
def run_eda():
    ...
    muli_menu = df.columns[4 : ]
    choice_mulit_list = st.multiselect('컬럼을 2개 이상 선택하세요',muli_menu)

    if len(choice_mulit_list) >= 2 :
        st.dataframe(df[choice_mulit_list].corr(numeric_only = True))

        fig1 = plt.figure()
        sb.heatmap(data = df[choice_mulit_list].corr(numeric_only = True),vmin = -1, vmax = 1, cmap ='coolwarm',annot = True,fmt= '.2f',linewidths=0.8)
        st.pyplot(fig1)
    else :
        pass
